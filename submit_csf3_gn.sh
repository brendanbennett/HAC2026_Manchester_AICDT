#!/bin/bash --login
# The Gauss-Newton (non-convex) reconstruction on one CSF3 GPU: calibrate, correct the
# calibrating public model, correct every scored model, then select and check the submission.
# Modelled on submit_csf3.sh (module loads, scratch, the Makefile's venv/toolchain/check), but
# runs scripts/run_nonconvex.sh instead of the full LPD pipeline -- this needs a calibrated
...
#
# Every scripts/run_nonconvex.sh variable (CHANNEL, RESTARTS, PHASES, HOLD_OUT_GEOMS,
# CALIBRATE_STEPS, CALIBRATE_MODELS, CALIBRATION_MODEL, SCORED, OUT_DIR, DATA_DIR) can be set
# in the environment before calling this script; nothing here overrides them.
#
...
#
# No real per-candidate timing exists for this workload yet, unlike the GA branch's
# scripts/profile_exact_forward.py. Start with SCORED="4" (one model) to see what a real run
# costs on this GPU before trusting the 10h default below for all of RESTARTS x 8 models
# (the calibrating model plus 7 scored ones).
...
# rather than deleting it and letting `make data` re-fetch ~6GB for nothing -- and potentially
# diverging from what was already validated, since the organisers' hosted files can drift
# after a checked-in dataset/MANIFEST.sha256 was cut. submit_csf3.sh's own scratch setup does
# not do this (rm -rf's an already-populated dataset/raw unconditionally); worth the same fix
# there too.
_scratch_link() {
  local path="$1" target="$2"
  if [ -L "$path" ]; then return; fi
  if [ -d "$path" ] && [ -n "$(ls -A "$path" 2>/dev/null)" ]; then
    echo "$path already has content -- moving it into scratch instead of re-fetching"
...
# still-loadable instrument exists), then reconstructs the calibrating model and every scored
# model, then selects and checks the submission. Every variable it reads (CHANNEL, RESTARTS,
# PHASES, HOLD_OUT_GEOMS, CALIBRATE_STEPS, CALIBRATE_MODELS, CALIBRATION_MODEL, SCORED,
# OUT_DIR, DATA_DIR) can be set in this script's own environment before it runs, per that
# script's own docstring -- nothing is duplicated here.
...
echo "=== done; results in ${OUT_DIR:-results/gn}/ (this machine) ==="
echo "Pull them to your laptop with (run FROM the laptop):"
echo "  rsync -avz \$USER@csf3.itservices.manchester.ac.uk:~/hac26/${OUT_DIR:-results/gn}/ \\"
echo "      /Users/user/Desktop/hac-2026/${OUT_DIR:-results/gn}/"
