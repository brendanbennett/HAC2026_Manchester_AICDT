...
class LPDNet(nn.Module):
    def __init__(self, op: ConvexPhotometricOperator, n_theta: int, n_phi: int,
        ...
        # --- occlusion gate ---------------------------------------------------------
        # Self-occlusion and cast shadow enter the true forward model as an entrywise
        ...
        #
        # K = A is unchanged, so every iteration is still one Chambolle-Pock step of a
        # genuine saddle-point problem -- unlike gating A itself, which makes K vary.
        #
        # R is bounded by identifiability, not compute: R*N unknowns against C*m
        # measurements, so R <~ 17 here. Sweep small R on held-out data.
        #
        # d1 must live in [0,1]: it is a VISIBILITY factor, and the model is only defined
        # -- forward and adjoint alike -- while it is one. An unconstrained d1 lets
        # y = sum_r d1^(r) (A w_r) go negative; normalize() then hits its clamp_min(eps)
        # floor with eps = 1e-3, turning a negative mean into a ~1e3x amplification, and
        # dn_adjoint's fallback branch multiplies the backward pass by 1/eps on top. That
        # is what produced the NaNs at R = 2.
        #
        # So d1 = sigmoid(.) in (0,1) identically, for every parameter value and every
        # input -- no clamp, no branch, no way to leave the set. With A >= 0 and w >= 0 that makes
