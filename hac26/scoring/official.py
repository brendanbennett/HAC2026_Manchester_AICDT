#!/usr/bin/env python3
"""The organisers' own two measures, as released in the challenge data.

`dataset/raw/Evaluation_measures/` holds the code the challenge will be scored with: a Python
voxel measure and a MATLAB projection measure. This module runs the first verbatim and ports
the second, so a reconstruction can be scored here with the same arithmetic rather than with
our approximation of it.

Two things about the released code are worth knowing before reading any score out of it.

**The projection measure only ever looks down the rotation axis.** `twoDmetric.m` rotates both
meshes about z by theta and then projects onto the xy plane. A rotation about z followed by a
projection onto xy is an in-plane rotation of one and the same silhouette, so theta changes the
picture's orientation and nothing else: every theta scores the top-down outline.
`projection_score` reproduces that, and `projection_score_axis` offers the reading the
challenge text implies instead -- genuine side views, the camera swung around the body -- so the
difference can be measured rather than assumed. Score against the code as released, because
that is what will be run, but do not let a recipe depend on the degeneracy.
...
`poly2mask`. Both are winding-blind. What the voxel measure is *not* blind to is a surface that
does not close: `.fill()` leaks through a hole. So an inside-out mesh is survivable and an open
one is not, which is the opposite of the intuition and the reason `scripts/check_submission.py`
tests for both.

Scoring conventions: the challenge sums, per model, one voxel score and one projection score,
each in [0, 1], higher better. The released voxel code returns *distances* (lower better);
`measure2` is 1 - Dice, so the challenge's published formula
`1 - (#(A\\B) + #(B\\A))/(#A + #B)` is `1 - measure2`.
"""
