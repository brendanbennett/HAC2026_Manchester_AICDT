...
from fix_pose import min_enclosing_radius                              # noqa: E402
...
def main():
    ...
    ap.add_argument("--no-fit-cylinder", dest="fit_cylinder", action="store_false",
                    help="skip imposing the published bounding-cylinder radius on export "
                         "(on by default; only useful for ablations)")
    ap.add_argument("--cylinder-fill", type=float, default=0.99,
                    help="fraction of the published R the exported body should reach")
    ...
    # Impose the published bounding-cylinder radius before export.
    #
    # This step was missing. `R` was read at the top and handed to the solver, but the
    # medoid mesh went straight to `export_stl`, so the exported width was whatever the
    # flow happened to produce. Measured on results/lpd, the ratio of achieved radius to
    # published R ran 0.22-1.01 across the ten models (model 10: 0.87 against 3.95), while
    # the convex branch -- which does call `fit_to_cylinder` via `eval_exact.decode` --
    # sat at 0.94-0.99 throughout.
    #
    # This is not a cosmetic pose issue. For a body scaled by s in xy and otherwise
    # contained in the truth, Dice = 2s^2/(1+s^2): s = 0.59 on model 2 caps the score at
    # 0.52 no matter how good the shape is, and model 2 measured exactly 0.5216. R is the
    # one degree of freedom the scale-free EGI inversion cannot recover -- the pose fixes
    # the height, R fixes the width -- so it has to be imposed, not hoped for.
    #
    # 0.99 R rather than R: measured on the public models the body reaches 0.98-0.99 of the
    # published bound, so it is a tight upper bound rather than an exact equality.
    if a.fit_cylinder:
        v = rescale_touch_z(np.asarray(v, float))
        r0, c = min_enclosing_radius(v[:, :2])
        v[:, 0] -= c[0]
        v[:, 1] -= c[1]
        v[:, :2] *= (a.cylinder_fill * R) / max(r0, 1e-12)
        print(f"  fitted to cylinder: R {r0:.3f} -> {a.cylinder_fill * R:.3f}", flush=True)
