...
def load_decoder(body, path="model/token_decoder.pt"):
    """Install the SHARED token decoder. Without this a fresh ImplicitBody carries random
    q/k/v/mlp weights, the code (p, z) is decoded by a decoder it was never fitted against,
    and the token field goes inert -- measured at Dice 1.0000 against the same body with its
    tokens switched off. It also made A(x) non-deterministic, since the random draw differed
    on every call."""
...
def curves_from_code(code, radius, surro, psi, res=32, device=None, geoms=None,
    ...
    # training step slower than everything else in the loop combined; the 0.33 s figure I
    ...
    if len(f) < 8:
        return None
    # Decimate to the token count the surrogate was TRAINED on. A 32^3 extraction yields
    # ~3300 facets, and tokenising all of them is both inconsistent with training and the
    # dominant cost here -- ray tracing scales as tokens x phases, so 3300 x 96 x 2 is
    # 640k rays per evaluation.
...
def corpus(n, psi, surro, device, seed=0, cache=None,
           codes_file="model/corpus_codes.npz"):
...
def main():
    ...
    ap.add_argument("--out", default="model/lpd_flow.pt")
    a = ap.parse_args()
